...
cc.Class({
    ...
    callCheckAssets: function (event) {
    ...
    callUpdateAssets: function (event) {
        var needRestart = false;
        var failed = false;

        this.panel.debugText.string = event.getEventCode().toString();
        switch (event.getEventCode())
        {
            case jsb.EventAssetsManager.ERROR_NO_LOCAL_MANIFEST:
                this.panel.info.string = 'No local manifest file found, hot update skipped.';
            ...
            case jsb.EventAssetsManager.UPDATE_FINISHED:
                ...
                this.panel.fileProgress.progress = 1;
                needRestart = true;
                break;
            case jsb.EventAssetsManager.UPDATE_FAILED:
                this.panel.info.string = 'Update failed msg' + event.getMessage();
        ...
        //if (needRestart) {
            ...
            this.panel.debugText.string = SON.stringify(searchPaths);
    ...
    checkIsNeedUpdate: function () {
        ...
        this._assetsManager.setEventCallback(this.callCheckAssets.bind(this));
    ...
    hotUpdate: function () {
        if (this._assetsManager && !this._updating) {
            this._assetsManager.setEventCallback(this.callUpdateAssets.bind(this));

            if (this._assetsManager.getState() === jsb.AssetsManager.State.UNINITED) {
                // Resolve md5 url
                var url = this.manifestUrl.nativeUrl;
                this.panel.debugText.string = url;
    ...
    onLoad: function () {
        ...
        var self = this;
        this.myUpdateBtn.on(cc.Node.EventType.TOUCH_END, function () {
            self.myUpdate();
        }, this);
        ...
        this._assetsManager = new jsb.AssetsManager(this.manifestUrl.nativeUrl, this._storagePath, this.versionCompareHandle);
